...
async function generate() {
	const url = env.NEXT_PUBLIC_RDFPROXY_ENDPOINT;
	assert(url, "NEXT_PUBLIC_RDFPROXY_ENDPOINT environment variable not provided.");

	const ast = await openapi(new URL("openapi.json", url), {
		arrayLength: true,
	});
